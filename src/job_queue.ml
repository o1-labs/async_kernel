  ...
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      Fields.iter
        ...
        ~jobs_left_this_cycle:
          (check (fun jobs_left_this_cycle ->
...
let set (type a) t i execution_context f a =
  ...
  A.unsafe_set t.jobs offset
    (Obj.repr (execution_context : Execution_context.t));
  ...
  try
    ...
    while can_run_a_job t do
      ...
      (* It is OK if [run_job] or [run_external_jobs] raises, in which case the exn is
         ...
         clearing [t]. *)
      !Tracing.tracers.on_job_enter execution_context;
